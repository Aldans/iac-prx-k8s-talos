...
# saving the multi-minute re-download on the next apply. A version bump
# (var.talos_version) re-downloads transparently because the file_name changes.
#
# To actually delete the image: temporarily comment out the lifecycle block
# (or run `terraform state rm proxmox_virtual_environment_download_file.talos_nocloud_image`),
