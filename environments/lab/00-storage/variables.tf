###############################################################################
# Proxmox (shared with the cluster project)
###############################################################################
...
variable "prx" {
  ...
  sensitive   = true
  description = "Proxmox API endpoint and credentials. Same shape as in kubernetes_iac/."
}
...
variable "dns_domain" {
  type        = string
  description = "DNS domain (must match the dnsmasq domain= on the Proxmox host). Same value as in kubernetes_iac/."
  default     = "lab.lan"
}
...
variable "zot_version" {
  type        = string
  description = "Zot registry release version (https://github.com/project-zot/zot/releases). v2.1.3+ is required for working sync onDemand (PR #2903 + #3156)."
  default     = "v2.1.16"
}
...
variable "garage_buckets" {
  type        = list(string)
  description = "Buckets to create in Garage. terraform-state and oci-blobs are required for the cluster project; the rest reserve room for Velero, Loki, etcd snapshots."
  default = [
    "terraform-state",
    "oci-blobs",
    "velero-backups",
...
variable "garage_state_bucket" {
  type        = string
  description = "Bucket name used by the cluster project as a Terraform S3 backend."
...
variable "admin_ssh_pubkey" {
  type        = string
  description = "SSH public key authorised for the 'admin' user on the storage VM. Used for troubleshooting only — Terraform itself does not SSH in."
  sensitive   = true
}
