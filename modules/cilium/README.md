...
| <a name="input_monitoring_enabled"></a> [monitoring\_enabled](#input\_monitoring\_enabled) | Enable Cilium + Hubble Prometheus metrics and render ServiceMonitor objects for them. Requires the Prometheus Operator CRDs to be present in the cluster first. Flipping this rolls the Cilium data plane (see rollout\_on\_values\_change). | `bool` | `false` | no |
